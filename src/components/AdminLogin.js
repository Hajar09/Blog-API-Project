...
class AdminLogin extends Component{
  constructor(props) {
    ...
    handleOnChange = (e) => {//récupére les valeurs de l'input et les met dans le state
           this.setState({
    ...
    submit = (e)=>{//comparer les valeurs de l'input aux valeurs de la base de données
      e.preventDefault();
      const login= {
        email:this.state.email,
        password:this.state.password
      }
          axios.post('http://localhost:5676/api/admin/login', login)//compare
          .then(response=>{//ici c'est la validation
            this.setState({
              login:response
            })
            this.props.history.push("/admin")//c'est la redirection
          })
          .catch(err =>{throw err})
    }
